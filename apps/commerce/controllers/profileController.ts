...
export const getAllProfiles = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const { page = 1, perPage = 10, profileName = '', email = '' } = req.query || {}
    const filters = {
...
export const getProfileById = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
...
// Delete a profile
export const deleteProfile = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
